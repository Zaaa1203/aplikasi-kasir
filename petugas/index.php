...
<div class="container">
	<div class="row">
			<div class="col-sm-8"><h3>Daftar User</h3></div>
			<div class="col-sm-4">
    <form class="d-flex" method="GET" action="">
        <input 
            ...
            list="petugasNames" 
            autocomplete="off" 
            value="<?= isset($_GET['np']) ? $_GET['np'] : '' ?>"
        >
        <button class="btn btn-primary" type="submit">Search</button>
    </form>
    <datalist id="petugasNames">
        <?php  
        // Connect to the database and fetch product names for suggestions
        include "../config.php";
        $sqlp = "SELECT username FROM petugas";
        $resp = mysqli_query($koneksi, $sqlp);
        while ($dtp = mysqli_fetch_array($resp)) {
        ?>
            <option value="<?= $dtp['username'] ?>"></option>
        <?php
        }
        ?>
    </datalist>
...
<table class="table table-hover table-striped table-sm">
	...
	<tbody>
		...
		<tr>
				...
				<td align="right">
					<a href="" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#modaledit<?= $data['id_petugas'] ?>">Edit</a>

					<a class="btn btn-danger btn-sm" href="delete.php?id=<?= $data['id_petugas'] ?>" onclick="return confirm('Apakah Anda Yakin data Petugas <?= $data['nama_petugas'] ?> akan dihapus ?')" class="hapus">Delete</a>
				</td>
		</tr>
			...
			<td colspan="6" class="text-center">
				<div class="container">
					<div class="row">
						...
						<!-- tombol input	 -->
							<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#myModal">[+] Data Petugas</button>						
						</div>
